...
클래스 다이어그램(Class Diagram)은 UML 중 가장 많이 사용하게 되는 다이어그램이 아닐까 싶네요.
몇 가지 형식들만 알면 클래스 다이어그램을 사용하는데는 큰 문제가 없습니다.

가장 많이 쓰이는 상속, 연관 2가지 정도만 알아도 대략적인 소통은 되니깐요.
...
클래스 속성(Attribute)이나 함수(Operation)들은 잘 표기를 안하지만 그래도 중요한 요소들에
대해서는 표기를 하기도 합니다.
